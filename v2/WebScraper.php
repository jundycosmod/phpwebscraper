...
class WebScraper {
    ...
    public function getItems($path, $pages)
    {
        $items = [];
        $this->stores = [];
        for($i = 1; $i <= $pages; $i++){
            $pageQuery = $i ? "&ref=pagination&page=".$i : "";
            $html = $this->getHtmlContents($this->baseUrl.$path, $this->query.$pageQuery, "GET");
    ...
    public function getStores($path){

        $storeDetails = [];
        foreach($this->stores as $store){
            $html = $this->getHtmlContents($this->baseUrl.$path. $store, "", "GET");
    ...
    public function saveStores($path){
        $csv = fopen('stores.csv', 'w');
        $stores = $this->getStores($path);
    ...
    public function saveItems($path, $numberOfPages){
        $csv = fopen('items.csv', 'w');
        $items = $this->getItems($path, $numberOfPages);
